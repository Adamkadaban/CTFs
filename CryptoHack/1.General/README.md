...
## Transparency
* The problem says Google enforces certificate transparency, which means we can look up certificates
* [This](https://transparencyreport.google.com/?hl=en) website let's us look up certificates
	* We can type in `cryptohack.org` and select `include subdomains`
* Looking through the certificates, one of the subdomains is `https://thetransparencyflagishere.cryptohack.org/`
* We can then get the flag with `curl -s https://thetransparencyflagishere.cryptohack.org/ | egrep -o crypto{.*?}`

* The flag is `crypto{thx_redpwn_for_inspiration}`
